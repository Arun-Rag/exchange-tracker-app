...
children.push(new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:60},
 children:[new TextRun({text:"OPTION 1 — Power Apps + SharePoint + Power Automate",size:26,color:BRAND2})]}));
...
children.push(table(
 ...
 [["Know what I gave the customer vs. what they gave me","Two clearly separated sections on every record: “Unit sold to customer” (P/N, S/N, Sales Order) and “Core received from customer” (P/N, S/N)."],
  ...
  ["Auto reminder email to customer, CC three colleagues","A scheduled Power Automate flow, fired at 30 days overdue; the three CC addresses are set once and reused."],
  ...
  ["Three return categories instead of a tick","A single Order Status choice — Open, Partial or Closed — shown as a coloured label. Partial covers part-returned orders (e.g. some cores still held at another stock location such as Kenya)."],
  ["Auto reminder when 30 days overdue","A scheduled Power Automate flow emails the customer once an open/partial exchange passes 30 days, CC’ing your three colleagues."]],
 [3400,5960]));
children.push(new Paragraph({children:[new PageBreak()]}));
...
children.push(table(
 ...
 [["Customer","Choice (or Lookup to Customers list)","The owner / company. Use the cleaned names: Advantage, Amapola, BusyBee, CC&Sons, I-Fly, Jetways, Kush, Newworld Choices, Sky Cap, Skyward Express, Tanzania, Aerosum, Aerojet, Orion."],
  ...
  ["Stock Location","Single line of text","For partial returns: where the part-returned unit is held (e.g. Kenya)."],
  ["Comment","Multiple lines of text","The sales-unit comment box."],
  ["Order Status","Choice","Open / Partial / Closed. Default Open. Partial = some cores returned, some still owed."],
  ["Reminder Sent","Date","Set automatically by the flow; stops duplicate reminders."]],
 [2100,2100,5160]));
children.push(callout("About “Due”","You do not create a “Due” column. “Due” is simply: Order Status = Open or Partial AND Exchange Status is still “Waiting for Exchange Unit” (or “Received – awaiting RO”). We build a filtered view for it in Section 4, so it is always live and never needs manual updating."));

children.push(H2("List B — Customers (pick-list)"));
children.push(P("One column (Title = customer name) plus a “Contact Email” column (this is where reminders are sent) — the reminder flow uses this to know where to send. Pre-fill it with the email address for each company."));
...
children.push(bulletR([{text:"Status labels, not ticks. ",bold:true},{text:"Show Order Status as a coloured pill: green “Closed”, blue “Partial”, amber “Open”, and a red “overdue” flag once 30 days pass."}]));
children.push(bulletR([{text:"Top counters. ",bold:true},{text:"Add cards across the top: Overdue (30 days+), Open, Partial, Closed, and Failed nozzles to claim — the same headline numbers as the prototype."}]));
children.push(bulletR([{text:"Search & filter. ",bold:true},{text:"A search box over P/N, S/N, customer, RO and SO; plus filter buttons All / Overdue / Open / Partial / Closed / Nozzle claims."}]));
...
children.push(bulletR([{text:"Core received from customer: ",bold:true},{text:"Core P/N, Core S/N, Exchange Status, Stock Location, Ownership."}]));
...
children.push(new Paragraph({shading:{fill:"F3F4F6",type:ShadingType.CLEAR},border:{left:{style:BorderStyle.SINGLE,size:14,color:BRAND2}},spacing:{before:60,after:60},
 children:[new TextRun({text:'Filter(Exchanges, (\'Order Status\'.Value = "Open" || \'Order Status\'.Value = "Partial") && DateDiff(Date, Today()) > 30)',font:"Consolas",size:18})]}));
...
children.push(P("This is built in Power Automate. It runs on a schedule, finds every open or partial exchange that has passed 30 days, and emails the customer — copying your three colleagues — without anyone clicking a button."));
...
children.push(new Paragraph({shading:{fill:"F3F4F6",type:ShadingType.CLEAR},border:{left:{style:BorderStyle.SINGLE,size:14,color:BRAND2}},spacing:{before:60,after:120},
 children:[new TextRun({text:"(OrderStatus eq 'Open' or OrderStatus eq 'Partial') and Date lt '@{addDays(utcNow(),-30)}'",font:"Consolas",size:18})]}));
...
children.push(new Paragraph({numbering:{reference:"sub",level:0},spacing:{after:50},children:[new TextRun({text:"CC: your three colleagues — set once here: thomas.carlborg@amapola.nu; mohsen.kharazi@amapola.nu; arun.raghavan@amapola.nu.",})]}));
...
Packer.toBuffer(doc).then(b=>{fs.writeFileSync("Option1_PowerApps_SharePoint_Guide.docx",b);console.log("docx bytes:",b.length);});
